...
Mail-siden viser ett døgn med post om gangen, slik den så ut da nattjobben gikk
klokka sju. Hver rad er en mail komprimert til fire linjer: hvem som sendte,
når, hva emnet var, og én setning om hva mailen faktisk vil. Det som krever noe
av Ole — et svar, en betaling, en frist — får oransje kant og en merkelapp som
sier hva, og står øverst i sin egen bolk. Resten kan skummes. Pilene i toppen
...
Endepunktet `/api/mail-digest` svarer bare ut avsender, tid, emne og setningen;
innholdet i posten forlater aldri Mac-en.
...
Tre sider i begge retninger — Nå, AI og Status. Retningen styrer bare hvordan
hver side deles opp, ikke hvilke sider som finnes: liggende legger dem i to
spalter, stående under hverandre.
...
- **Nå** — klokke og dato på én linje, «Nå» og «Neste» som to like kort med
  ...
  fokusøkt og knapperaden. I liggende står klokka og avtalene i venstre spalte,
  musikk, fokusøkt og knapper i høyre.
- **AI** — kvotene for Codex og Claude, og de nyeste agent-øktene. Kvotene har
  hele kortet for seg selv med leverandørene under hverandre: side ved side ble
  «7 dag…» kappet og «85 % igjen» brukket over to linjer. Gjenstående prosent
  står i 25 px, stolpene er 8 px, og nullstillingen står under hver.
...
Spørringen som skiller retningene står to steder — som mediespørring i
`mobile.css` og som `LANDSCAPE_QUERY` i `MobilePanel.jsx`. En test vokter at de
er like.
...
«Hold skjermen våken» er en egen bryter nederst på Nå-siden, under
hurtigknappene, og i liggende en egen rad i full bredde. Den er en bryter og
ikke et ikon med vilje: iOS kan nekte låsen, og da må forskjellen på «slått på»
og «faktisk våken» stå skrevet i stedet for å måtte gjettes ut av en farge.
Linja under navnet sier hvilken av de tre tilstandene som gjelder.
